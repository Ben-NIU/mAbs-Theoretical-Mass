...
Calc.Mass.HC.nonR<-function(HC, PyroE.hc, Lys, Glycan, AorB, Others, howmany,ds.hing, ds.lchc, ds.hchc){
 source("formula.R")
  ...
  OTHERS<-if(AorB=="Pre-defined"){list("+Lys"=c("C"=6, "H"=12,"N"=2, "O"=1, "S"=0), "none"=c(C=0, H=0, N=0, O=0, S=0), "+Glycation"=c(C=6, H=10, N=0, O=5, S=0), "Oxidation"=c(C=0, H=0, N=0, O=1, S=0), "Water-Loss"=c(C=0, H=-2, N=0, O=-1, S=0), "NH3-Loss"=c(C=0, H=-3, N=-1, O=0, S=0), "Deamidation"=c(C=0, H=-1, N=-1, O=1, S=0), "Cysteinylation"=c(C=3, H=5, N=1, O=2,S=1))} else {list("BB"=Turn(Others))}
...
total<-unlist(ConvertPeptide(HC, IAA=FALSE)) + pyroe + lys + GLYCAN[[Glycan]] + OTHERS[["BB"]]*howmany/2 + c(C=0, H=as.numeric(ds.hing)*(-1), N=0, O=0, S=0) + c(C=0, H=as.numeric(ds.lchc)*(-1), N=0, O=0,S=0) + c(C=0, H=as.numeric(ds.hchc)*(-1), N=0, O=0, S=0)
