...
Calc.Mass.nonR<-function(HC, LC, PyroE.hc, PyroE.lc, Lys, Glycan,DSB){
  ...
  total<-unlist(ConvertPeptide(HC, IAA=FALSE))*2 + unlist(ConvertPeptide(LC, IAA=FALSE))*2 + pyroe.hc*2 + pyroe.lc*2 +lys*2 + GLYCAN[[Glycan]]*2  + c(C=0, H=as.numeric(DSB)*(-2), N=0, O=0, S=0)
  return(total)
}