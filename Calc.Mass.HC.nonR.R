...
Calc.Mass.HC.nonR<-function(HC, PyroE.hc, Lys, Glycan, Others, howmany, cust, cust.ptm, c.howmany, ds.hing, ds.lchc, ds.hchc, extra){
 source("formula.R")
...
if(cust == "No"){  
  ...
  total<-unlist(ConvertPeptide(HC, IAA=FALSE)) + pyroe + lys + GLYCAN[[Glycan]] + OTHERS[[Others]]*howmany/2 + c(C=0, H=as.numeric(ds.hing)*(-1), N=0, O=0, S=0) + c(C=0, H=as.numeric(ds.lchc)*(-1), N=0, O=0,S=0) + c(C=0, H=as.numeric(ds.hchc)*(-1), N=0, O=0, S=0) + extra 
  
} 
else if(cust == "Yes"){

total<-unlist(ConvertPeptide(HC, IAA=FALSE)) + pyroe + lys + GLYCAN[[Glycan]] + OTHERS[[Others]]*howmany/2 + Turn(cust.Ptm, cust)*c.Howmany/2 + c(C=0, H=as.numeric(ds.hing)*(-1), N=0, O=0, S=0) + c(C=0, H=as.numeric(ds.lchc)*(-1), N=0, O=0,S=0) + c(C=0, H=as.numeric(ds.hchc)*(-1), N=0, O=0, S=0) + extra
