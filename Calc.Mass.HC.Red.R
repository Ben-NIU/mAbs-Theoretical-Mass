...
Calc.Mass.HC.Red<-function(HC, PyroE.hc, Lys, Glycan, AorB, Others, howmany, where, ds.hcun=3){
  ...
  E<-if(where=="HC"){1} else if(where=="LC"){0}
  ...
  if(AorB=="Pre-defined"){  
    ...
    total<-unlist(ConvertPeptide(HC, IAA=FALSE)) + pyroe + lys + GLYCAN[[Glycan]] + X*howmany*E + c(C=0, H=as.numeric(ds.hcun)*(-2), N=0, O=0, S=0) 
